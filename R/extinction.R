extinction <- function(web, participant="both", method="random"){
  ...
  #        to true abundance); partial matching of strings allowed
  ...
  meths <- c("random", "abundance")
  meths.match <- pmatch(method, meths)
  if (is.na(meths.match)) stop("Choose extinction method: random/abundance.\n")
