...
functspec <- function(web, quantitative=FALSE, inf.replace=NA){
  ...
  list(lower=FSlower, higher=FShigher)
...
# apply(replicate(100, functspec(matrix(sample((web>0)*1), nrow=nrow(web), ncol=ncol(web)))$higher), 2, max)
#max(replicate(100, functspec(matrix(sample((web>0)*1), nrow=nrow(web), ncol=ncol(web)))$higher))